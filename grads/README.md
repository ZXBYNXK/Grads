...
- [x] Set the view engine for express to pug
      ...
      `javascript app.set("views", path.join(__dirname, 'views'));`
      &nbsp;
...
- [x] Create a bash script outside of the project, a quick tool to set enviroment variables.
      ...
      `bash #!/bin/bash export DB=yourDatabaseConnectionStringHere export PORT=setPort export NODE_ENV=development echo 'Set the enviroment to development'`
      // Ctrl + s (Save) <br>
      // Ctrl + x (Exit) <br>
...
- [x] Now assign the enviroment variables value using 'process.env'
      ...
      `javascript const PORT = process.env.PORT, DB = process.env.DB;` - [x] Now use the above values to connect to cloud or local database & set server to listen on a port.
...
- [x] Write to 'home_route.js' and create an instance of an express router to be exported
      ...
      `javascript const express = require('express'), router = express.Router();`
      &nbsp;
...
- [x] Export the schema with the mognoose.model() syntax
      ...
      `javascript module.exports = mongoose.model('Graduate', GraduateSchema);`
      &nbsp;
...
- [x] Import the model in routes/home_route.js
      ...
      `javascript const Graduate = require('../models/graduate');`
      &nbsp;
      &nbsp;
...
* [x] Create a GET route for the root route for the '/api/home' endpoint and have it render index.pug
      ...
      `javascript router.get('/', (req, res) => { res.status(200).render('index'); });`
      &nbsp;
...
* [x] Create a POST route to the same endpoint '/api/home' for new graduates.
      ...
      `` javascript router.post('/', async (req, res) => { const newUser = new Graduate(req.body); try { console.log('Attempting post request.'); const savedUser = await newUser.save(); res.status(201).json({ message: `Thank you ${req.body.firstname}!` }); } catch (error) { res.status(401).json({ message: 'Please fill out the required feilds' }); } }); ``
      &nbsp;
...
* [x] Create a GET route with an extended endpoint 'api/home/all'.
      ...
      `javascript router.get('/all', async (req, res) => { try { const allGraduates = await Graduate.find(); res.status(200).json(allGraduates); } catch (err) { res.status(500).render('errors', { error: '500 Server error.' }); } });`
      &nbsp;
      &nbsp;
